const Catchphrase = () => {
  return (
    // <div className="flex w-full flex-col gap-4 p-4 py-36 pr-0 sm:px-10 sm:pt-24 md:px-0 md:pt-60 bg-red-500 lg:pb-20 h-[calc(100vh-4rem)] xl:h-[50rem] lg:pr-0 lg:pl-10 lg:pt-10 xl:pt-40">
    <div className="flex h-[calc(100vh-4rem)] w-full flex-col justify-center gap-10 p-4 pb-40 pr-0 pt-20 lg:h-[40rem] lg:gap-4 lg:pb-10 xl:h-[calc(100vh-4rem)]">
      <h1 className="font-italic text-xl sm:text-2xl md:text-3xl xl:text-4xl">
        Elevate Your Living Experience
      </h1>
      <h1 className="mb-4 ml-10 text-5xl text-30 md:mb-10 md:ml-12 md:text-6xl lg:ml-20 lg:text-7xl xl:ml-24 xl:text-8xl">
        Interior Exterior Designer
      </h1>
      <div className="ml-24 lg:ml-40 xl:ml-48">
        <h1 className="font-italic text-3xl sm:text-4xl md:text-5xl lg:text-6xl">
          Designing Dreams,
        </h1>
        <h1 className="font-italic text-3xl sm:text-4xl md:text-5xl lg:text-6xl">
          One Room at a Time
        </h1>
      </div>
    </div>
  )
}

export default Catchphrase
